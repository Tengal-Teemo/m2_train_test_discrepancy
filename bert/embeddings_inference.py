...
class OpenAI_Encoder:
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], batch_size: int, **kwargs) -> np.ndarray:
        ...
        passages = [cutoff_long_text_for_embedding_generation(passage, self.encoding, cutoff=8192) for passage in
                    passages]
...
class Cohere_Encoder:
    ...
    def encode_queries(self, queries: List[str], batch_size: int, **kwargs) -> np.ndarray:
        ...
        for query_chunks in tqdm(split_list(queries, self.encoder_batch_size)):
            try:
                encoded_queries = self.co.embed(texts=query_chunks, model=self.embedding_model,
                                                input_type="search_query", truncate=self.truncation)
            except:
                time.sleep(30)
                encoded_queries = self.co.embed(texts=query_chunks, model=self.embedding_model,
                                                input_type="search_query", truncate=self.truncation)
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], batch_size: int, **kwargs) -> np.ndarray:
        ...
        for passage_chunks in tqdm(split_list(passages, self.encoder_batch_size)):
            try:
                encoded_passages = self.co.embed(texts=passage_chunks, model=self.embedding_model,
                                                 input_type="search_document", truncate=self.truncation)
            except:
                time.sleep(30)
                encoded_passages = self.co.embed(texts=passage_chunks, model=self.embedding_model,
                                                 input_type="search_document", truncate=self.truncation)
...
class M2_BERT_Encoder:
    ...
    def encode_queries(self, queries: List[str], batch_size: int, **kwargs) -> np.ndarray:
        ...
        for queries_chunk in tqdm(split_list(queries, batch_size),
                                  total=len(queries) // batch_size + (1 if len(queries) % batch_size != 0 else 0)):
            ...
            for query in queries_chunk:
                ...
                if final_input_ids == None:
                    ...
                    final_input_ids['attention_mask'] = torch.cat(
                        (final_input_ids['attention_mask'], input_ids['attention_mask']), 0)
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], batch_size: int, **kwargs) -> np.ndarray:
        ...
        for passages_chunk in tqdm(split_list(corpus, batch_size),
                                   total=len(corpus) // batch_size + (1 if len(corpus) % batch_size != 0 else 0)):
...
class MiniLM6Encoder:
    ...
    def encode_queries(self, queries: List[str], batch_size: int, **kwargs) -> np.ndarray:
        encoded_passages = self.encoder.encode(queries)
        return np.array(encoded_passages)

    def encode_corpus(self, corpus: List[Dict[str, str]], batch_size: int, **kwargs) -> np.ndarray:
        if self.cfg.get("evaluation_max_seq_len") >= 512:
            passages_chunk = [passage['title'] + " " + passage['text'] for passage in corpus]
        else:
            passages_chunk = [passage['text'] for passage in corpus]

        encoded_passages = self.encoder.encode(passages_chunk)
        return np.array(encoded_passages)


class M2_BERT_Raw_encoder:
    ...
    def encode_strings(self, strings: list[str], batch_size: int) -> np.ndarray:
        embeddings = []
        for sublist_strings in tqdm(split_list(strings, batch_size),
                                  total=len(strings) // batch_size + (1 if len(strings) % batch_size != 0 else 0)):
    ...
    def encode_queries(self, queries: List[str], batch_size: int, **kwargs) -> np.ndarray:
        encoded_passages = self.encode_strings(queries, batch_size=batch_size)
        return np.array(encoded_passages)
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], batch_size: int, **kwargs) -> np.ndarray:
        ...
        encoded_passages = self.encode_strings(passages_chunk, batch_size=batch_size)
        return np.array(encoded_passages)
...
class Together_Encoder:
    ...
    def encode_queries(self, queries: List[str], batch_size: int, **kwargs) -> np.ndarray:
        ...
        total_encoded_queries = []
        for query_chunks in tqdm(split_list(queries, self.encoder_batch_size),
                                 total=len(queries) // self.encoder_batch_size + (
                                 1 if len(queries) % self.encoder_batch_size != 0 else 0)):
            try:
                encoded_queries = generate_together_embeddings(text=query_chunks[0],
                                                               model_api_string=self.together_model_name,
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], batch_size: int, **kwargs) -> np.ndarray:
        ...
        total_encoded_passages = []
        for passage_chunks in tqdm(split_list(passages, self.encoder_batch_size),
                                   total=len(passages) // self.encoder_batch_size + (
                                   1 if len(passages) % self.encoder_batch_size != 0 else 0)):
            # breakpoint()
            try:
                encoded_passages = generate_together_embeddings(text=passage_chunks[0],
                                                                model_api_string=self.together_model_name,
                                                                api_key=self.api_key)
            except:
                time.sleep(30)
